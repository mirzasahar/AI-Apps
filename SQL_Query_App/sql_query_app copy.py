...
def generate_sql_query(user_query, db_schema):
    ...
    2. For text aggregation:
       - Use STRING_AGG for combining text fields
       - Include relevant GROUP BY fields
    
    3. For ratios and rates:
       - Calculate as CAST(numerator AS FLOAT) / NULLIF(denominator, 0)
       - Include both raw counts and calculated ratios
    
    EXAMPLE ANALYTICAL QUERY:
    SELECT TOP 100
        REPLACE(REPLACE(o.ownerid, '{{', ''), '}}', '') as clean_ownerid,
        owner.fullname as sales_rep_name,
        COUNT(DISTINCT o.opportunityid) as total_opportunities,
        COUNT(DISTINCT CASE WHEN o.new_dropoutreason IS NOT NULL THEN o.opportunityid END) as dropped_opportunities,
        COUNT(DISTINCT l.leadid) as total_leads,
        CAST(COUNT(DISTINCT o.opportunityid) AS FLOAT) / NULLIF(COUNT(DISTINCT l.leadid), 0) as opportunity_to_lead_ratio,
        CAST(COUNT(DISTINCT CASE WHEN o.new_dropoutreason IS NOT NULL THEN o.opportunityid END) AS FLOAT) 
            / NULLIF(COUNT(DISTINCT o.opportunityid), 0) as dropout_rate
    FROM DynamicsShortlisted.dbo.opportunity o
    LEFT JOIN DynamicsShortlisted.dbo.owner owner
        ON REPLACE(REPLACE(o.ownerid, '{{', ''), '}}', '') = REPLACE(REPLACE(owner.ownerid, '{{', ''), '}}', '')
    LEFT JOIN DynamicsShortlisted.dbo.lead l
        ON REPLACE(REPLACE(o.xt_lead, '{{', ''), '}}', '') = REPLACE(REPLACE(l.leadid, '{{', ''), '}}', '')
    GROUP BY REPLACE(REPLACE(o.ownerid, '{{', ''), '}}', ''), owner.fullname
    HAVING COUNT(DISTINCT o.opportunityid) > 0
...
def retry_query_execution(query: str, max_retries: int = 3) -> Tuple[bool, Union[pd.DataFrame, str]]:
    """
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count == max_retries:
                error_msg = f"Query failed after {max_retries} attempts. Error: {str(e)}"
                return False, error_msg
            st.warning(f"Query failed (Attempt {retry_count}/{max_retries}). Retrying...")
...
def create_streamlit_app():
    ...
    if st.button("Generate and Execute Query"):
        ...
        with st.spinner("Generating SQL query..."):
            try:
                # Initial triage
                if not is_data_question(user_query):
                    response = generate_general_response(user_query)
                    st.write(response)
                    return
                
                # Check if query is in scope
                if not is_query_in_scope(user_query, db_schema):
                    st.error("Query is out of scope. Please ask a question related to the available data.")
                    return
                ...
                success, result = retry_query_execution(sql_query)
                ...
                if success:
                    if isinstance(result, pd.DataFrame) and not result.empty:
                        ...
                        with st.spinner("Analyzing results..."):
                            analysis = analyze_results(result, user_query)
                            st.write("Analysis:")
                            st.write(analysis)
                    else:
                        st.warning("Query executed successfully but returned no results.")
                else:
                    st.error(f"Failed to execute query: {result}")
                    
            except Exception as e:
                st.error(f"An error occurred: {str(e)}")

def is_data_question(query: str) -> bool:
    """
    Determine if the user's question requires data access
    
    Args:
        query: User's question
        
    Returns:
        bool: True if question requires data access, False if it's a general question
    """
    # List of keywords that indicate a data question
    data_keywords = [
        'how many', 'count', 'show me', 'list', 'find', 'search',
        'where', 'when', 'what', 'which', 'who', 'analyze',
        'compare', 'total', 'average', 'sum', 'minimum', 'maximum',
        'report', 'statistics', 'trend', 'distribution', 'breakdown'
    ]
    
    query_lower = query.lower()
    return any(keyword in query_lower for keyword in data_keywords)

def is_query_in_scope(query: str, schema: str) -> bool:
    """
    Check if the query is within the scope of available data
    ...
    available_tables = re.findall(r'table: (\w+)', schema_lower)
    
    # Keywords that might indicate out-of-scope queries
    out_of_scope_keywords = [
        'weather', 'stock', 'price', 'market',
        'social media', 'twitter', 'facebook',
        'external', 'third party', 'competitor'
    ]
    ...
    # Check if query mentions any available tables or their common variations
    return any(table in query_lower or table.replace('_', ' ') in query_lower 
              for table in available_tables)

def generate_general_response(query: str) -> str:
    """
    Generate a response for general questions that don't require data access
    ...
    """
    prompt = f"""
    You are a helpful assistant. Please provide a concise response to this general question:
    {query}
